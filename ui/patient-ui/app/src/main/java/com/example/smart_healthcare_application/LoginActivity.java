    ...
    public void login(String username, String password) {
        ...
        userRepository.login(username, password, new ApiCallback<User>() {
            ...
            public void onSuccess(User result) {
                ...
                // Lấy thông tin Patient ngay sau khi đăng nhập thành công
                PatientRepository.getInstance().getPatientByUserId(result.getUserId(), new ApiCallback<Patient>() {
                    @Override
                    public void onSuccess(Patient patient) {
                        Log.d("Login", "Fetch Patient success !!!");
                        LoginManager.getInstance(LoginActivity.this).savePatientSession(patient);
                        
                        // Sau khi lưu xong cả session user và patient, ta mới chuyển màn hình
                        proceedToMain();
                    }

                    @Override
                    public void onError(String errorMessage) {
                        Log.e("Login", "Fetch Patient failed: " + errorMessage);
                        Toast.makeText(LoginActivity.this, "Lỗi lấy hồ sơ: " + errorMessage, Toast.LENGTH_SHORT).show();
                        showLoading(false);
                    }
                });
